...
if (isset($_POST['tambah'])) {
    ...
    if($aksi == 'edit'){
        ...
        if(!empty($_FILES['foto_produk']['name'])) {
            ...
            $simpan = mysqli_query(
                $conn,
                "UPDATE produk set nama_produk='$nama_produk', harga='$harga', stok='$stok', kategori='$kategori', deskripsi='$deskripsi', foto='$foto' where id_produk='$id'"
            );
        } else {
            $simpan = mysqli_query(
                $conn,
                "UPDATE produk set nama_produk='$nama_produk', harga='$harga', stok='$stok', kategori='$kategori', deskripsi='$deskripsi' where id_produk='$id'"
            );
        }
    }else{
        move_uploaded_file($tmp, "img/$foto");
        ...
        $query = "INSERT INTO produk
            (nama_produk, harga, stok, kategori, deskripsi, foto)
            VALUES 
            ('$nama_produk', '$harga', '$stok', '$kategori', '$deskripsi', '$foto')";
...
<body>
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                ...
                <form method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" value="<?= @$nama ?>" name="nama_produk">
                    </div>
                    <div class="mb-3">
                        <label for="exampleInputPassword1" class="form-label">Harga</label>
                        <input type="number" class="form-control" value="<?= @$harga ?>" name="harga_produk">
                    </div>
                        <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label">Stok</label>
                        <input type="text" class="form-control" value="<?= @$stok ?>" name="stok_produk">
                    </div>
                    <div class="mb-3">
                        <label for="formFile" class="form-label">Foto</label>
                        <input class="form-control" type="file" name="foto_produk" id="formFile">
                        <input class="form-control" type="hidden" value="<?= @$foto ?>" name="foto_produk" id="formFile">
                    </div> 
                    <div class="mb-3">
                        <label for="form-select" class="form-label">Kategori</label>                   
                        <select class="form-select" name="kategori_produk" aria-label="Default select example">
                            <option selected><?= @$kategori ?></option>
                            <option value="1">Batik1</option>
                            <option value="2">Batik2</option>
                        </select>       
                    </div>             
                    <div class="mb-3">
                        <label for="exampleInputEmail1" class="form-label" value="<?= @$deskripsi ?>">Deskripsi</label>
            ...
            <table class="table table-striped table-sm">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td>
                      <img src="img/<?= $data['foto'] ?>" height="100px">
                    </td>
                    <td>
                      <a href="index2.php?aksi=edit&id=<?= $data['id_produk'] ?>" class="btn btn-sm btn-warning">Edit</a>
                      <a href="index2.php?aksi=delete&id=<?= $data['id_produk'] ?>" class="btn btn-sm btn-danger">Hapus</a>
                    </td>
    ...
    <script>
        function confirmDelete(id) {
            Swal.fire({
                title: 'Serius Ini Mau Dihapus?',
                text: "Data yang dihapus tidak dapat dikembalikan!",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#d63030ff',
                cancelButtonColor: '#305cd6ff',
                confirmButtonText: 'Ya, hapus!',
                cancelButtonText: 'Batal'
            }).then((result) => {
                if (result.isConfirmed) {
                    window.location.href = 'index2.php?aksi=delete&id=' + id;
                }
            });
        }
    </script>
